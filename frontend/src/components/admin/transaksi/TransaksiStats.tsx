...
function TransaksiStats() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      {stats.map((item, idx) => (
        <div key={idx} className={`${item.bg} p-6 rounded-[24px] border border-gray-100 shadow-sm transition-all hover:shadow-md`}>
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">{item.label}</p>
          <p className={`text-2xl font-bold ${item.color} mb-2`}>{item.value}</p>
          {item.trend && (
             <div className="flex items-center gap-1 text-[10px] text-green-500 font-medium">
               <TrendingUp size={12} /> {item.trend}
             </div>
          )}
          {item.sub && <p className="text-[10px] text-gray-400 font-medium">{item.sub}</p>}
        </div>
      ))}
    </div>
  )
