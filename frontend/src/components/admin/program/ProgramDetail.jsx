  ...
  useEffect(() => {
    const fetchProgramDetail = async () => {
      try {
        ...
        const isImageValid =
          data.image && data.image !== "null" && String(data.image).length > 50;
        ...
        alert(
          err.response?.data?.error ||
            "Program tidak ditemukan atau server bermasalah.",
        );
  ...
  const handleDelete = async () => {
    if (
      ...
      try {
        ...
        const msg =
          err.response?.data?.error || "Terjadi kesalahan saat menghapus data.";
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end">
              <div>
                ...
                <p className="text-2xl font-black text-[#7FAE5A]">
                  {program.progress}%
                </p>
              </div>

              <span className="text-sm text-gray-400 italic">
                Target: {program.formattedTarget}
              </span>
            </div>
